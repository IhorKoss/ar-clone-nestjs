...
export class AuthService {
  constructor(
    ...
    private readonly bannedEmailRepository: BannedEmailsRepository,
    private readonly refreshTokenRepository: RefreshTokenRepository,
  ) {}

  public async signUp(dto: SignUpReqDto): Promise<AuthResDto> {
    await this.isEmailNotExistOrThrow(dto.email);
    await this.isEmailBannedOrThrow(dto.email);
  ...
  public async createManager(dto: SignUpReqDto): Promise<UserResDto> {
    await this.isEmailNotExistOrThrow(dto.email);
    await this.isEmailBannedOrThrow(dto.email);
    const password = await bcrypt.hash(dto.password, 10);
    const user = await this.userRepository.save(
  ...
  private async isEmailBannedOrThrow(email: string) {
    const bannedEmail = await this.bannedEmailRepository.findOneBy({ email });
    if (bannedEmail) {
